...
void main() {
  ...
  group('PageIntentClassifier Tests', () {
    ...
    test(
        'Magnet links are classified as magnetPage and openNewTabWithDownloadSuggestion',
        () {
      final res =
          classifier.classify('magnet:?xt=urn:btih:1234567890abcdef&dn=Ubuntu');
    ...
    test(
        'Torrent file URLs are classified as magnetPage and openNewTabWithDownloadSuggestion',
        () {
      final res =
          classifier.classify('https://example.com/files/ubuntu.torrent');
      expect(res.intent, PageIntent.magnetPage);
      expect(res.action, PageAction.openNewTabWithDownloadSuggestion);
    ...
    test(
        'Direct archive/executable download URLs are classified as directDownload',
        () {
    ...
    test(
        'Media files (mp4) are classified as mediaPage with download suggestion',
        () {
    ...
    test('Auth pages are classified as authPage and open in same tab', () {
      ...
      final oauth =
          classifier.classify('https://accounts.google.com/o/oauth2/auth');
      expect(oauth.intent, PageIntent.authPage);
      expect(oauth.action, PageAction.openSameTab);
    });

    test(
        'Download pages / File hosting are classified as filePage and open in same tab',
        () {
      final res =
          classifier.classify('https://mediafire.com/file/12345/document');
      expect(res.intent, PageIntent.filePage);
