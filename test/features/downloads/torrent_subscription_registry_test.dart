...
void main() {
  ...
  group('TorrentSubscriptionRegistry & Handler Hardening (Sprint 1)', () {
    test(
        'Registry tracks handler strongly and cleans up subscription on unregister',
        () async {
    ...
    test(
        'Adaptive sync interval returns 30s for small torrents and 120s for huge torrents',
        () {
      expect(TorrentDownloadHandler.computeAdaptiveSyncInterval(500),
          equals(const Duration(seconds: 15)));
      expect(TorrentDownloadHandler.computeAdaptiveSyncInterval(1500),
          equals(const Duration(seconds: 30)));
      expect(TorrentDownloadHandler.computeAdaptiveSyncInterval(12000),
