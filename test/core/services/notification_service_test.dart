...
void main() {
  ...
  group('NotificationService Action Stream & Queue', () {
    ...
    test('Rapid subscribe and unsubscribe does not lose active events',
      ...
      service
          .dispatchActionForTest({'action': 'pause', 'taskId': 'task_rap_1'});

      final sub1 = service.onActionTapped.listen((_) {});
      await sub1.cancel();

      service
          .dispatchActionForTest({'action': 'resume', 'taskId': 'task_rap_2'});
    ...
    test('Concurrent action submission is handled safely', () async {
      ...
      await Future.wait([
        Future(() => service.dispatchActionForTest(
            {'action': 'pause', 'taskId': 'concurrent_1'})),
        Future(() => service.dispatchActionForTest(
            {'action': 'resume', 'taskId': 'concurrent_2'})),
        Future(() => service.dispatchActionForTest(
            {'action': 'stop', 'taskId': 'concurrent_3'})),
      ]);
