...
void main() {
  group('CycleState domain tests', () {
    ...
    test('fromLibtorrent handles seedingEnabled parameter correctly', () {
      expect(CycleState.fromLibtorrent('seeding', seedingEnabled: true), CycleState.seeding);
      expect(CycleState.fromLibtorrent('seeding', seedingEnabled: false), CycleState.completed);
      expect(CycleState.fromLibtorrent('stalled_uploading', seedingEnabled: true), CycleState.seeding);
    });
    ...
    test('fromLibtorrent fuzzy/substring matching works as expected', () {
      expect(CycleState.fromLibtorrent('is_seeding_now', seedingEnabled: true), CycleState.seeding);
      expect(CycleState.fromLibtorrent('is_seeding_now', seedingEnabled: false), CycleState.completed);
      expect(CycleState.fromLibtorrent('checking_resume_data_v2'), CycleState.verifying);
      expect(CycleState.fromLibtorrent('downloading_metadata_fast'), CycleState.fetchingMetadata);
