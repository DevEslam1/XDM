...
void main() {
  ...
  group('Production Readiness Audit Fixes', () {
    test('L-4: PauseReason fallback parsing handles unknown strings safely',
      ...
      expect(PauseReason.fromName('non_existent_reason'),
          equals(PauseReason.unknown));
      expect(PauseReason.fromName(null, fallback: PauseReason.user),
          equals(PauseReason.user));
      expect(
          PauseReason.fromName('networkLost'), equals(PauseReason.networkLost));
    });
    ...
    test(
        'L-3: TaskCompanionConverter.isInterruptedActiveRow identifies stale running rows',
        () {
    ...
    test(
        'D-7: Multi-threaded downloaded byte calculation never falls back to preallocated file length',
        () async {
    ...
    test(
        'D-13: TorrentSeedingManager calculates ratio safely with fallback bytes for magnets',
        () {
    ...
    test(
        'D-14: FFmpegMuxService calculates dynamic timeout according to media size',
        () {
      final zeroTimeout = FFmpegMuxService.dynamicTimeout(0);
      expect(zeroTimeout, equals(const Duration(minutes: 10)));

      final smallTimeout =
          FFmpegMuxService.dynamicTimeout(10 * 1024 * 1024); // 10MB
      expect(smallTimeout, equals(const Duration(minutes: 11)));

      final largeTimeout =
          FFmpegMuxService.dynamicTimeout(20 * 1024 * 1024 * 1024); // 20GB
