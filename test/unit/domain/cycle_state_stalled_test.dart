...
void main() {
  group('CycleState.fromLibtorrent Unknown Label Remediation (P1-4)', () {
    test('returns CycleState.stalled for unknown labels to surface failures',
        () {
      expect(CycleState.fromLibtorrent('corrupt_internal_state'),
          CycleState.stalled);
      expect(CycleState.fromLibtorrent('unrecognized_engine_code'),
          CycleState.stalled);
      expect(CycleState.fromLibtorrent('stuck_tracker_lookup'),
          CycleState.stalled);
    });
