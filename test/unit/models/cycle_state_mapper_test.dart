...
void main() {
  group('CycleState mapper', () {
    test('maps libtorrent state strings to CycleState accurately', () {
      ...
      expect(CycleState.fromLibtorrent('queued_for_checking'), CycleState.verifying);
    ...
    test('handles null and unknown labels gracefully', () {
      ...
      expect(CycleState.fromLibtorrent('some_random_state'), CycleState.downloading);
    });

    test('parses names in camelCase and snake_case', () {
      expect(CycleState.fromName('fetchingMetadata'), CycleState.fetchingMetadata);
      expect(CycleState.fromName('fetching_metadata'), CycleState.fetchingMetadata);
      ...
      expect(CycleState.fromName('non_existent', fallback: CycleState.paused), CycleState.paused);
    });
  });
}
