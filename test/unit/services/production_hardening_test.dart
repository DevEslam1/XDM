...
class _TestWidgetState extends State<_TestWidget> with SafeDisposalMixin {
  ...
  void initState() {
    ...
    sub = trackSubscription(
        Stream<int>.periodic(const Duration(milliseconds: 100), (i) => i)
            .listen((_) {}));
    timer =
        trackTimer(Timer.periodic(const Duration(milliseconds: 100), (_) {}));
...
void main() {
  group('Production Readiness Hardening Tests', () {
    testWidgets(
        'SafeDisposalMixin safely tracks and cleans up timers, streams, notifiers on dispose',
        (tester) async {
    ...
    test(
        'UrlValidator correctly filters dangerous schemes and enforces safety bounds',
        () {
      // Valid URLs
      ...
      expect(
          UrlValidator.isValid(
              'magnet:?xt=urn:btih:d143c08e5c1d6837910ff61dc970fb9aa2d83b63'),
          isTrue);
      ...
      expect(
          UrlValidator.isValid('https://example.com/${'a' * 2100}'), isFalse);
      ...
      // Sanitization
      expect(UrlValidator.sanitize(' https://example.com/\x00file.zip '),
          equals('https://example.com/file.zip'));
    });
  });
}
