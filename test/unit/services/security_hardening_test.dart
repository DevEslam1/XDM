...
void main() {
  ...
  group('Task 9: Security Hardening & Secret Defense Suite', () {
    test(
        'LoggingService.sanitize strictly redacts bearer tokens, secrets, and auth headers',
        () {
      const rawText =
    ...
    test('DiagnosticService redacts sensitive tokens upon entry recording', () {
      DiagnosticService.instance.record(
          'AUTH_EVENT', 'User authenticated with api_key=secret-key-xyz-12345');

      final logs = DiagnosticService.instance.entries;
      final hasSecret =
          logs.any((entry) => entry.message.contains('secret-key-xyz-12345'));

      expect(hasSecret, isFalse,
          reason:
              'DiagnosticService must sanitize all messages to prevent secret retention');
    });
    ...
    test(
        'URL sanitization strips sensitive query params (key, token, secret, auth)',
        () {
      const testUrl =
          'https://cdn.example.com/download/file.zip?token=secret123&auth=abcdef&name=file';
