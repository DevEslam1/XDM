...
void main() {
  ...
  group('TorrentResumeStore Unit Tests', () {
    ...
    test('registerSource and unregisterTorrent manage in-memory registry', () {
      ...
      TorrentResumeStore.registerSource(102, 'magnet:?xt=urn:btih:ABCDEF1234567890ABCDEF1234567890ABCDEF12');
    ...
    test('saveAndWait and loadResumeDataForSource store and verify blob durably', () async {
      const sourceUrl = 'magnet:?xt=urn:btih:1234567890abcdef1234567890abcdef12345678&dn=Test';
      ...
      final loadedBlob = await TorrentResumeStore.loadResumeDataForSource(sourceUrl);
      ...
      final afterDelete = await TorrentResumeStore.loadResumeDataForSource(sourceUrl);
      expect(afterDelete, isNull);
    });

    test('saveAll batch saves all registered torrent IDs', () async {
      const urlA = 'magnet:?xt=urn:btih:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
      const urlB = 'magnet:?xt=urn:btih:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb';
    ...
    test('loadResumeDataForSource rejects empty / invalid URLs gracefully', () async {
      final res = await TorrentResumeStore.loadResumeDataForSource('non_existent_url_123');
      expect(res, isNull);
    });

    test('extracts info-hash from .torrent file on disk when available', () async {
      ...
      final loaded = await TorrentResumeStore.loadResumeDataForSource(torrentFilePath);
