...
void main() {
  ...
  group('Task 5: Friendly Localized Errors Suite', () {
    test(
        'Formats integrity failure as friendly localized message without raw exception dump',
        () {
      const exc = DownloadIntegrityException(
          'CRC32 mismatch on chunk #4 at byte 0x40000');
      ...
      expect(result.message, isNot(contains('0x40000')),
          reason:
              'Technical details must not be exposed in user-facing message');
    ...
    test('Formats disk write failure without raw errno or paths', () {
      const exc = PositionalFileWriterException(
          'EACCES: permission denied, open /private/var/...');
      final result = ErrorTaxonomy.classify(exc);
    ...
    test('Formats disk out-of-space error clearly', () {
      const exc =
          PositionalFileWriterException('ENOSPC: no space left on device');
      final result = ErrorTaxonomy.classify(exc);
    ...
    test('Formats URL expiration as friendly renewal prompt', () {
      const exc =
          UrlExpiredException('Signature expired at timestamp 1700000000');
      final result = ErrorTaxonomy.classify(exc);
