  ...
  group('Task 6: Feature Availability Safety Suite', () {
    test('TorrentService stub reports unsupported and unavailable', () {
      ...
      expect(TorrentService.isAvailable.value, isFalse,
          reason:
              'Stubbed torrent service availability notifier must be false');
      expect(TorrentService.addMagnet('magnet:?xt=urn:btih:012345', '/tmp'),
          equals(-1));
      expect(TorrentService.activeTorrentIds, isEmpty);
    });

    test('RemoteApiService is disabled by default and starts no server',
        () async {
      expect(RemoteApiService.enabled, isFalse,
          reason:
              'Remote API server must be disabled by default for zero attack surface');
