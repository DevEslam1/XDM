  ...
  Future<void> _startTask(DownloadTask task) async {
    ...
    final downloadFuture = _downloadEngine
        ...
        .then((_) async {
          ...
          if (hasAudio) {
            final currentAfterVideo = _findTask(task.id);
            if (currentAfterVideo == null ||
                currentAfterVideo.status != DownloadStatus.downloading) {
              return;
            }

            audioTempPath = '${currentAfterVideo.tempFilePath}.audio';

            // Download audio stream
            await _setTask(currentAfterVideo.copyWith(errorMessage: 'Downloading audio...'));
            final audioCancelToken = CancelToken();
            _cancelTokens[task.id] = audioCancelToken;

            debugPrint('[DMX] Audio download starting: ${task.mergedAudioUrl}');
            await _downloadEngine.download(
              url: task.mergedAudioUrl!,
              tempFilePath: audioTempPath,
              localFilePath: audioTempPath,
              knownFileSize: currentAfterVideo.audioSize,
              supportsResume: true,
        ...
        .catchError((Object error) async {
          ...
          // Cleanup audio temp file if it exists
          final audioTempPath = '${current.tempFilePath}.audio';
          final af = File(audioTempPath);
          if (await af.exists()) {
            try {
              await af.delete();
