...
class DownloadProvider extends ChangeNotifier
  ...
  Future<void> _startTask(DownloadTask task) async {
    ...
    final videoFuture = _downloadEngine
        .download(
          url: task.url,
    ...
    if (hasAudio) {
      audioFuture = _downloadEngine.download(
    ...
    // Run video and audio concurrently. If one fails, cancel the other and
    // surface the real error (not a wrapped ParallelWaitError).
    Object? firstError;
    final downloadFuture = Future.wait([
      ...
      if (audioFuture != null)
        audioFuture.catchError((e) {
          // Only save error if not caused by a cancel from our own video failure
