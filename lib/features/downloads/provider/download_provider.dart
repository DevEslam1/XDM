...
class DownloadProvider extends ChangeNotifier
        ...
        .then((_) async {
          if (task.mergedAudioUrl != null) {
            ...
            if (current != null && current.status == DownloadStatus.downloading) {
              await _setTask(current.copyWith(errorMessage: 'Downloading audio for merge...'));
              final audioTempPath = '${task.tempFilePath}.audio';
              final audioCancelToken = CancelToken();
              _cancelTokens[task.id] = audioCancelToken;
              ...
              await _setTask(current.copyWith(errorMessage: 'Merging video and audio...'));
              ...
              final success = await FFmpegMuxService.mergeVideoAudio(currentForMerge.localFilePath, audioTempPath, mergedPath);
              
              if (success) {
                final localFile = File(currentForMerge.localFilePath);
                if (await localFile.exists()) await localFile.delete();
                final audioFile = File(audioTempPath);
                if (await audioFile.exists()) await audioFile.delete();
                await File(mergedPath).rename(currentForMerge.localFilePath);
              } else {
          ...
          // Clear any merge messages
          await _setTask(current.copyWith(clearError: true));
          final now = DateTime.now();
  ...
  Future<void> updateTaskUrl(String taskId, String newUrl, {String? newAudioUrl}) async {
    final index = _tasks.indexWhere((t) => t.id == taskId);
    if (index == -1) return;
