  ...
  Future<void> download({
    ...
    if (isTorrent) {
      ...
      cancelToken.whenCancel.then((_) async {
        ...
        debugPrint('Torrent metadata cancel handler error: $e');
      });
      ...
      if (torrentFiles != null && torrentFiles.isNotEmpty) {
        final priorities = torrentFiles.map((f) {
      ...
      cancelToken.whenCancel.then((_) {
        // Guard against firing after the download already finished — otherwise
        // we would pause a torrent ID that may have been reused by another task.
        if (downloadCompleter.isCompleted) return;
        downloadSub?.cancel();
        TorrentService.pauseTorrent(id);
        if (!downloadCompleter.isCompleted) {
