...
class DownloadEngine {
  ...
  Future<void> download({
    ...
    final isTorrent = isTorrentUrl(url, fileName: p.basename(currentLocalFilePath));
  ...
  Future<void> _handleTorrentDownload({
    ...
    metadataTimer = Timer(const Duration(seconds: 120), () {
      ...
      if (!cancelToken.isCancelled) {
        try {
          ...
          debugPrint('[DMX] Error removing torrent during metadata timeout: $e');
        }
      }
      if (!metadataCompleter.isCompleted) {
        metadataCompleter.completeError(
    ...
    final saveDir = File(currentLocalFilePath).parent.path;
    final alreadyOnDisk = Directory(saveDir).existsSync() &&
        scanFolderBytesSync(saveDir) > 0;
      ...
      if (!isMultiThread) {
        ...
        if (canResume) {
          try {
            ...
            if (decoded is Map) {
              final savedTotalSize = (decoded['totalSize'] as num?)?.toInt() ?? -1;
              final savedThreadCount = (decoded['threadCount'] as num?)?.toInt() ?? -1;
              ...
              if (savedTotalSize == totalSize && 
                ...
                for (int i = 0; i < threadCount; i++) {
                  if (!await partFiles[i].exists()) { canResume = false; break; }
                }
        ...
        if (!canResume) {
          ...
          for (int i = 0; i < threadCount; i++) {
            ...
            // Truncate part file to the exact downloaded length to prevent gaps on append
            final raf = await partFiles[i].open(mode: FileMode.write);
            await raf.truncate(chunkProgress[i]);
            await raf.close();
          }
        }
        ...
        try {
                  ...
                  try {
                    ...
                    try {
                      await for (final chunk in stream) {
                        ...
                        // Reset every iteration regardless of whether a
                        // limit is active right now — see the matching fix
                        // in the single-threaded path for why leaving this
                        // inside `limit > 0` caused a huge stall the first
                        // time throttling was turned on mid-download.
          ...
          // Concatenate part files into the temp file via streams to avoid OOM on
          // large files (never readAsBytes()).
          ...
          for (final pf in partFiles) {
            try { if (await pf.exists()) await pf.delete(); } catch (_) {}
          }
          ...
          if (totalSize > 0) {
            final downloadedChunkSum = chunkProgress
                ...
                .toInt();
            final difference = (downloadedChunkSum - totalSize).abs();
            // Use the same ratio-based tolerance as the single-threaded path
            // (see the check after `_downloadSingleThreaded`'s write loop),
            // so a given mismatch isn't rejected in one mode and silently
            // accepted in the other depending on which code path ran.
            final ratio = totalSize > 0 ? difference / totalSize : 1.0;
            if (difference > 1024 && ratio > 0.001) {
              throw Exception(
                'Download integrity check failed: expected $totalSize bytes, got $downloadedChunkSum.',
              );
            }
          }
  ...
  Future<void> _downloadSingleThreaded({
    ...
    final postRedirectName = responseName ?? fileNameFromUrl(response.realUri.toString());

    final currentName = p.basename(localFilePath);
    final bool isGeneric = currentName.toLowerCase() == 'download' ||
  ...
  Future<void> _waitForState(
    ...
    sub = TorrentService.torrentUpdates.listen((torrents) {
      ...
      if (tor != null && predicate(tor.stateLabel.toLowerCase())) {
        if (!completer.isCompleted) completer.complete();
      }
    });
    t = Timer(timeout, () {
  ...
  void close() {
    ...
    for (final id in _activeTorrentIds) {
      try { TorrentService.pauseTorrent(id); } catch (_) {}
    }
