...
// ═══════════════════════════════════════════════════════════════════════════
// LEGACY JOURNAL — read-only compatibility shim.
//
// The v3 engine does not WRITE journals: atomic state writes on a 2 s cadence
// plus transition-time saves bound worst-case crash loss to ~2 s of bytes,
// and disk reconciliation repairs everything else. This class exists to
// recover progress from journals written by older builds during migration.
// ═══════════════════════════════════════════════════════════════════════════

class DownloadJournal {
  DownloadJournal(this.path, {int? compactionThresholdBytes});

  final String path;

  /// Opens for append. No-op in the v3 engine (kept for API compatibility).
  Future<void> open() async {}

  Future<void> writeInit(int threadCount, int totalSize) async {}

  Future<void> recordChunkProgress(int chunkIndex, int bytes) async {}

  Future<void> writeCheckpoint(List<int> progress, int totalSize) async {}
  ...
  /// Best-effort recovery of per-chunk byte counts from a legacy journal.
  ///
  /// Accepts JSON-lines checkpoints (`{"t":"ckpt","progress":[..]}`) and
  /// per-chunk records (`{"t":"chunk","i":N,"bytes":B}`); returns the most
  /// progress that can be proven, or null when nothing readable exists.
    ...
    try {
      ...
      for (final line in lines) {
        ...
        try {
          ...
          final t = decoded['t'];
          if (t == 'ckpt' && decoded['progress'] is List) {
            final snapshot = (decoded['progress'] as List)
                .map((e) => e is num ? e.toInt() : 0)
                .map((b) => b < 0 ? 0 : b)
                .toList();
            final sum = snapshot.fold<int>(0, (s, b) => s + b);
            if (sum > bestSum) {
              bestSum = sum;
              best = snapshot;
            }
          } else if (t == 'chunk') {
            final i = (decoded['i'] as num?)?.toInt();
            final b = (decoded['bytes'] as num?)?.toInt();
            if (i != null && b != null && i >= 0 && b >= 0) {
              final prev = perChunk[i] ?? 0;
              if (b > prev) perChunk[i] = b;
            }
          }
        } catch (_) {
          // Skip unparseable line — journals are advisory.
        }
      }
      if (best != null) return best;
      if (perChunk.isEmpty) return null;
      final maxIdx = perChunk.keys.reduce((a, b) => a > b ? a : b);
      return List<int>.generate(maxIdx + 1, (i) => perChunk[i] ?? 0);
    } catch (e) {
      debugPrint('[DmxState] journal recover failed: $e');
      return null;
    }
  }
